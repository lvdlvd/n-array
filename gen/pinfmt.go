...
type symbols struct {
	...
	dev  *Device // for physical pin-number lookup
}
...
func buildSymbols(d *Device) symbols {
	s := symbols{mux: map[string]muxInfo{}, pin: map[string]bool{}, flag: map[string]flagInfo{}, dev: d}
...
type columns struct {
	num                string // physical pin number for the selected package, "" if none/bulk
...
func (s symbols) decode(expr string) columns {
	...
	num := ""
	if selPackage != "" && len(pins) == 1 { // a single pin has one physical number
		num = s.dev.pinNumber(pins[0], selPackage)
		if num == "" {
			num = "n/e" // pin exists in the family but is not bonded out on this package
		}
	}
	return columns{num: num, pin: strings.Join(pins, "|"), fn: funcStr, af: afStr, attrs: strings.Join(attrs, ","), ok: ok}
}
...
func pinfmtFile(d *Device, path string) error {
	...
	var wCode, wNum, wPin, wFn, wAf, wAttrs int
	for _, e := range entries {
		wCode = max(wCode, len(e.code))
		wNum = max(wNum, len(e.cols.num))
		wPin, wFn, wAf = max(wPin, len(e.cols.pin)), max(wFn, len(e.cols.fn)), max(wAf, len(e.cols.af))
		wAttrs = max(wAttrs, len(e.cols.attrs))
	}

	// rewrite aligned — code column, then derived columns, then human. The pin#
	// column (selPackage) leads the derived block when present.
	changed := 0
	for _, e := range entries {
		attrs := e.cols.attrs
		if e.human != "" {
		...
		if wNum > 0 {
			col = fmt.Sprintf("%*s  %s", wNum, e.cols.num, col) // right-aligned pin number
		}
	...
	for _, e := range entries {
		if e.cols.num == "n/e" {
			ne = append(ne, e.cols.pin)
		}
	}
	if len(ne) > 0 {
		fmt.Fprintf(os.Stderr, "narray: not exposed on %s (n/e): %s\n", selPackage, strings.Join(ne, ", "))
	}
	return nil
}
...
func pinoutMarkdown(d *Device, path string, w io.Writer) error {
	...
	num := selPackage != ""
	if num {
		p("Package: %s.\n\n| Pin | Port | Function | AF | Config | Connected to |\n", selPackage)
		p("| --- | ---- | -------- | -- | ------ | ------------ |\n")
	} else {
		p("| Port | Function | AF | Config | Connected to |\n")
		p("| ---- | -------- | -- | ------ | ------------ |\n")
	}
	for _, e := range entries {
		...
		if num {
			p("| %s | %s | %s | %s | %s | %s |\n", e.cols.num, pin, e.cols.fn, af, e.cols.attrs, e.human)
		} else {
